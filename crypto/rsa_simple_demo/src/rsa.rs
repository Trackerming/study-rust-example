use crypto_util::base_compute::{chinese_remainder_theorem, mod_exp, mod_inverse};
use rand::Rng;
...
impl RSA {
    ...
    pub fn decrypt_crt(&self, private_key: usize, cipher: usize) -> usize {
        let d_p = private_key % (self.p - 1);
        let d_q = private_key % (self.q - 1);
        println!("cipher: {cipher}, d_p = {d_p}, d_q= {d_q}");
        let m_p = mod_exp(cipher, d_p, self.p);
        let m_q = mod_exp(cipher, d_q, self.q);
        let result = chinese_remainder_theorem(&[m_p, m_q], &[self.p, self.q]);
        result
    }
...
mod test {
    ...
    #[test]
    fn test_encrypt_decrypt_crt() {
        let rsa = RSA::new(17, 11);
        let key_pair = rsa.generate_key_pair();
        println!("key_pair: {:?}", key_pair);
        let msg = 66;
        let cipher = rsa.encrypt(key_pair.0, msg);
        let plaintext = rsa.decrypt_crt(key_pair.1, cipher);
        println!(
            "decrypt by crt cipher: {}, plaintext: {}",
            cipher, plaintext
        );
        assert_eq!(plaintext, msg);
    }
